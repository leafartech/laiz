...
export default function HeroFlex({ children, imagePath, reverse, title, subtitle, topTitle, href, cta }: HeroFlexProps) {
    return (
        <div className={`flex flex-col px-4 lg:px-0 items-center gap-4 sm:gap-12 ${reverse ? 'sm:flex-row-reverse' : 'sm:flex-row'}`}>
            <div className="w-full sm:w-1/2 flex flex-col gap-2">
                <h2 className="font-bold text-2xl sm:text-3xl text-zinc-950">{title}</h2>
                {children}
                {href && (
            ...
            <div className={`w-full sm:w-1/2 flex ${reverse ? 'justify-start' : 'justify-end'}`}>
                <img src={`./images/${imagePath}.png`} alt={title} className="rounded-2xl max-h-96" />
            </div>
        </div>
    )
}
